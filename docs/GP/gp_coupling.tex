...
to D-Flow FM. The aquifer--sewer seepage is two-way: it is computed from the
...
by physical position. Within a user time step SWMM advances first, so that its
...
before D-Flow FM advances; the second and third user time steps repeat this and
are drawn faded, with the exchange shown only once. At the end of each coupling
interval the D-Flow FM water level and water depth, $s_1$ and $h_s$, are mapped to
the MODFLOW~6 coastal boundary; MODFLOW~6 advances; and the simulated boundary
flow, $q^{\mathrm{ext}}$, is returned to D-Flow FM. The aquifer--sewer seepage,
$Q_j$, is drawn with two heads and in a neutral color because it belongs to
neither model: it is computed from the MODFLOW~6 head and the SWMM water surface
in the pipe together, and applied to both as a specified flux, equal and opposite.
The exchange column occupies no model time and is given width
only to separate the arrows. Three user time steps per coupling interval is the
ratio at the finest interval simulated here; it reaches 288 at daily coupling.}
\label{fig:sequence}
\end{figure}
