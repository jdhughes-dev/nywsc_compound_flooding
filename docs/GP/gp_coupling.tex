...
\caption{Order of execution and state exchanged among the three simulators over
two coupling intervals. Lanes are ordered by execution rather than by physical
position. On each user time step, $\Delta t_u$, SWMM advances and its mean outfall
discharge, $\bar{q}_s$, is written into the D-Flow FM source term, after which
D-Flow FM advances. At the end of each coupling interval, $\Delta t_c$, the D-Flow
FM water level and water depth, $s_1$ and $h_s$, are mapped to the MODFLOW~6
coastal boundary; MODFLOW~6 advances one time step; and the simulated boundary
...
only to separate the arrows. The coupling interval is drawn as three user time
steps, which is the ratio at the finest interval simulated here; it reaches 288 at
daily coupling.}
\label{fig:sequence}
\end{figure}
