...
MODFLOW~6 is advanced one time step, and the simulated boundary flows and the
aquifer--sewer seepage computed from the simulated heads are returned to D-Flow FM
and to SWMM. Because each exchange uses the state at the end of the preceding
...
flow, $q^{\mathrm{ext}}$, and aquifer--sewer seepage, $Q_j$, are returned to
D-Flow FM and SWMM. The exchange column occupies no model time and is given width
