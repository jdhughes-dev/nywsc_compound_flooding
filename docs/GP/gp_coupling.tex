...
this study bounds. $Q_j$ is drawn in a neutral color, and as two horizontal runs
joined by a double-headed riser, because it belongs to neither model: it is
computed at the exchange from the MODFLOW~6 head and the SWMM water surface in the
pipe together, and applied to the next step of both as a specified flux, equal and
opposite. The exchange break occupies no model time---both of its edges carry the
