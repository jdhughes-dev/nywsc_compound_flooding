...
component. Exchange formulations are given for each pair of models. Particular
...
SWMM is controlled through the PySWMM interface \citep{mcdonnell2020pyswmm}. The
...
Explicit coupling is a choice rather than a constraint of the interfaces. The
MODFLOW application programming interface exposes the nonlinear solution within a
time step, so an exchange can in principle be repeated and the coupled state
brought to convergence before the models advance \citep{hughes2022modflowapi}. An
implicit coupling of that kind would be far less sensitive to the exchange
interval, which is the quantity the demonstration below measures. It is not
attempted here: the explicit scheme is the simpler starting point, and quantifying
how the explicit solution depends on the exchange interval is a prerequisite for
judging what an implicit scheme would be worth.
...
%   - an implicit coupling, iterating the exchange to convergence within a time
%     step through the MODFLOW API's access to the nonlinear solution, and what
%     the measured interval sensitivity implies it would be worth
