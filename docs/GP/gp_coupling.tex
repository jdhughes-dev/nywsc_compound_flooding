...
as a line between two models, because it belongs to neither: a single collector
gathers the end-of-step MODFLOW~6 head and SWMM water surface in the pipe and
enters $Q_j$ as one arrow, since only the difference between them is ever used,
and two arms deliver the resulting flux, equal and opposite, to the start of the
next step in each. One consequence is worth reading off the figure directly---a single
