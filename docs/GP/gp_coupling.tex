...
by physical position. Each arrow ends on the model time step that \emph{uses} the
value, so the lean of an arrow shows whether the value is applied backward or
forward in time. Within a user time step SWMM advances first, so that its
outfall covers the interval D-Flow FM is about to integrate, and the mean outfall
discharge over that step, $\bar{q}_s$, is written into the D-Flow FM source term
before D-Flow FM advances; that arrow is vertical because there is no lag. The
second and third user time steps repeat this and are drawn faded, with the
exchange shown only once. At the end of each coupling interval the D-Flow FM water
level and water depth, $s_1$ and $h_s$, are mapped to the MODFLOW~6 coastal
boundary; that arrow leans back because the state read at the end of the interval
is the boundary condition for the MODFLOW~6 step over the interval that just
ended, consistent with the backward-in-time step MODFLOW~6 takes internally.
MODFLOW~6 then advances, and the boundary flow, $q^{\mathrm{ext}}$, and the
aquifer--sewer seepage, $Q_j$, are computed from the new solution and held over
the interval about to start; those arrows lean forward, and that one-interval lag
is the coupling error the sweep in this study measures. $Q_j$ is drawn with two
heads and in a neutral color because it belongs to neither model: it is computed
from the MODFLOW~6 head and the SWMM water surface in the pipe together, and
applied to both as a specified flux, equal and opposite. The exchange column
occupies no model time---both of its edges carry the same tick---and is given
width only to separate the arrows. Three user time steps per coupling interval is
the ratio at the finest interval simulated here; it reaches 288 at daily
coupling.}
\label{fig:sequence}
\end{figure}
