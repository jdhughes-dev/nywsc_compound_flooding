...
\title{In-memory coupling of coastal hydrodynamic, groundwater flow, and sewer
network models through their library interfaces}
...
simulation. Exchange formulations are given for each pair of models. Particular
attention is given to returning the sewer model outfall discharge to the
hydrodynamic model as a sewage tracer source, which places the strongest
...
rather than sampled. The approach is demonstrated on a sanitary sewer network on
...
Standardized control interfaces make that possible. Simulators that expose a
library interface, such as the Basic Model Interface \citep{hutton2020bmi} or an
application programming interface built for the same purpose
\citep{hughes2022modflowapi}, can be loaded into a single process and advanced
under external control, with state passed as arrays in memory. The exchange
interval is then set by the numerics rather than by any intermediate file format,
and a self-consistent coupled solution is obtained from one simulation. What such
an interface exposes, however, is not always sufficient on its own: a quantity
may be readable but not writable in a way the solution uses, and where within the
model time step a value is written can determine whether it survives to be used
at all.
...
the hydrodynamic model as a sewage tracer source is treated in most detail,
...
once. Sanitary sewers in low-lying coastal communities are subject to groundwater
...
joints, increasing the volume conveyed to the treatment works and discharged at
outfalls. The water table is itself tidally forced, so the infiltration rate is
modulated at tidal frequencies by a process external to the sewer network. The
demonstration uses a sanitary sewer network at Greenport, New York, on the north
fork of eastern Long Island, which discharges to Greenport Harbor and the
surrounding waters of Long Island Sound and Peconic Bay. It is used here to
evaluate the coupling rather than to characterize the site, and in particular to
establish how frequently the exchange must occur for the coupled solution to be
insensitive to the exchange interval.

\section*{Model components}

Three simulators are coupled. Coastal hydrodynamics and the transport of a
conservative sewage tracer are simulated with D-Flow Flexible Mesh (D-Flow FM),
which solves the depth-averaged shallow water equations on an unstructured grid
\citep{deltares2024dflowfm}. Groundwater flow is simulated with MODFLOW~6
...
associated with the saltwater interface are represented. Flow in the sanitary
sewer network is simulated with the Storm Water Management Model (SWMM)
\citep{rossman2015swmm}.

The three simulators are advanced within a single process and exchange state
through their respective library interfaces. MODFLOW~6 is controlled through
its application programming interface \citep{hughes2022modflowapi}, and D-Flow
FM through its Basic Model Interface implementation \citep{hutton2020bmi}.
SWMM is controlled through the PySWMM interface \citep{mcdonnell2020pyswmm}.
Model construction and post-processing use FloPy \citep{hughes2023flopy}.

\section*{Coupling framework}

The coupled simulation is driven by the D-Flow FM model time. D-Flow FM advances
in increments of its user time step, $\Delta t_u$, and the remaining models are
advanced and exchanged at a coupling interval, $\Delta t_c$, that is an integer
multiple of $\Delta t_u$. Within a coupling interval the sequence of operations
is: D-Flow FM is advanced to the end of the interval; the resulting water
levels and water depths are mapped to the MODFLOW~6 coastal boundary; MODFLOW~6
is advanced one time step; the simulated boundary flows are returned to D-Flow
FM as an external volume source; simulated groundwater heads are used to compute
the sewer exchange; and SWMM is advanced. Because each exchange uses the state
at the end of the preceding operation, the coupling is fully explicit and no
iteration is performed within a coupling interval.
...
\section*{Sewage tracer transport}

Flow reaching the outfall is introduced to D-Flow FM as a point source carrying
a conservative sewage tracer, which is then advected and dispersed by the
...
The coupling is exercised on the Greenport sewer network described above. Three
D-Flow FM grids of increasing resolution were used, spanning the same domain and
driven by the same tidal and atmospheric forcing. The groundwater and sewer
...
% interface must meet for this kind of coupling, including the model-time-step
% window the SWMM outfall discharge transfer depends on. Forward-looking
% paragraphs on applying the same pattern to other simulator combinations close
% the section.
