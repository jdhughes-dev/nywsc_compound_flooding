...
Panel A shows a ten-day window rather than the whole simulation. At 15-minute
coupling the rate has 8,544 samples over 89 days, which drawn in full is a solid
band; a window is the only way the tidal signal, and the collapse of its amplitude
as the interval lengthens, are visible at all. Panel B shows the whole run, because
the point it makes is that the cumulative curves end up on top of one another.
"""
...
# what panel A shows is the tidal signal and not an event.
WINDOW = (40.0, 50.0)
...
def make(refresh=True):
    ...
    with styles.USGSPlot():
        ...
        for key in ORDER:
            ...
            axA.plot(t[w], rate[w], "-", color=colors[key], lw=0.9,
                     label=LABEL[key])
            axB.plot(t[m], cum[m], "-", color=colors[key], lw=1.0)

        axA.axhline(0.0, color="0.35", lw=0.7, linestyle=(0, (3, 2)), zorder=0)
        axA.set_xlim(*WINDOW)
        for ax in (axA, axB):
        ...
        axA.annotate(f"root-mean-square rate falls from {rms['15.00M']:.1f} to "
        ...
        axB.annotate(f"cumulative spans {span:.1f} percent across the same intervals",
